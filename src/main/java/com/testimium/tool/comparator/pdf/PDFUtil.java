...
public class PDFUtil {
	...
	private TextPartialComparator comparatorTool;
	private ComparatorResponse comparatorResponse;
   ...
   */   
	...
	/**
	 *
	 * @param comparatorTool
	 */
	public void setComparatorTool(TextPartialComparator comparatorTool) {
		this.comparatorTool = comparatorTool;
	}

	public ComparatorResponse getComparatorResponse() {
		return this.comparatorResponse;
	}
   ...
   */	
	...
	private boolean comparepdfFilesWithTextMode(String file1, String file2, int startPage, int endPage) throws IOException{
		this.comparatorResponse = null;
		String file1Txt = this.getPDFText(file1, startPage, endPage).trim();
		String file2Txt = this.getPDFText(file2, startPage, endPage).trim();

		//Adding exclude date patten into exclude patter
		/*String[] newExcludedPattern = null;
		int excludePatternSize = 0;
		if(null != this.excludeDatePattern && excludeDatePattern.size() > 0) {
			if(null != this.excludePattern && this.excludePattern.length > 0) {
				excludePatternSize = this.excludePattern.length;
				 newExcludedPattern = new String[excludePatternSize + this.excludeDatePattern.size()];

				for (int i = 0; i < excludePatternSize; i++) {
					newExcludedPattern[i] = this.excludePattern[i];
				}
			} else {
				newExcludedPattern = new String[this.excludeDatePattern.size()];
			}
			...
			this.excludePattern = newExcludedPattern;
		}*/
		...
		comparatorTool.setOutputFileName("PdfPartialDiff");
		comparatorTool.setExcludePattern(this.excludePattern);*/
		comparatorTool.setExcludePattern(this.excludePattern);
		String actualTempFile = FileUtility.createTempFile("Actual", file1Txt);
		String expectedTempFile = FileUtility.createTempFile("Expected", file2Txt);
		//ComparatorResponse compRes = null;
		try {
			this.comparatorResponse = this.comparatorTool.compare(actualTempFile, expectedTempFile, assertKey);
		} catch (ToolComparatorException e) {
			//TODO Exception Handling
			logger.log(Level.SEVERE, "Exception PDFUtil.comparepdfFilesWithTextMode() " + e.getMessage(), e);
			this.comparatorResponse = new ComparatorResponse("FAIL", "Comparison Failed" + e.getMessage(), null);
		}
		...
		return this.comparatorResponse.getResult().equals("PASS") ? true : false;
	}
   ...
   */
	private boolean comparePdfByImage(String file1, String file2, int startPage, int endPage) throws IOException{
		...
		if(pgCount1!=pgCount2){
			...
			throw new IOException("Actual and expected files page counts do not match");
		}
	...
	private boolean convertToImageAndCompare(String file1, String file2, int startPage, int endPage) throws IOException{
		...
		try {
				...
				diffFileName = new String[startPage + endPage];
				for(int iPage=startPage-1;iPage<endPage;iPage++){
					...
					this.diffFileName[iPage + 1] = fileName;
