...
public class AssertParameter {
    ...
    /**
     * Return's the first assertion element from the assertion map.
     * @return
     */
    public Assertions getFirst() {

        Assertions assertion = null;
        if (null != this.getAssertParams() && this.getAssertParams().size() > 0) {
            for (Map.Entry<String, Assertions> entry : this.getAssertParams().entrySet()) {
                assertion = entry.getValue();
                break;
            }
        }
        return assertion;
    ...
    public Assertions getByAssetKey(String assertKey) {

        Assertions assertion = null;
        if (null != this.getAssertParams() && this.getAssertParams().size() > 0) {
            for (Map.Entry<String, Assertions> entry : this.getAssertParams().entrySet()) {
                if(entry.getKey().equals(assertKey)) {
                    assertion = entry.getValue();
                    break;
                }
            }
